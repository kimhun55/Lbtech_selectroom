...
//$pdf->Open();
$this->pdf->SetMargins(6,1.6,5); // SetMargins(float left, float top [, float right])
$this->pdf->rm="(No code)งานศูนย์ข้อมูล [".date("Y-m-d H:i:s")."]";
//$this->pdf->tp=$tp;
	...
	//$navi = "?na=canPer1";
		...
		for($x=0; $x<$num_group; $x++){
			...
			$teacher_lname = $rsGroup['teacher_lname'];
					...
					$this->pdf->Cell(20,5.5,iconv("UTF-8","TIS-620","ปีการศึกษา_____/2560"),0,1,'L');
					...
					$this->pdf->SetXY(88,5);
					$this->pdf->SetFont('angsana','B',13);
					$this->pdf->Cell(115.5,10,iconv("UTF-8","TIS-620",""),1,1,''); // กรอบสี่เหลี่ยมมุมบนขวามือ
					$this->pdf->SetXY(7,5);
					$this->pdf->Cell(285,5.5,iconv("UTF-8","TIS-620","วิทยาลัยเทคนิคลพบุรี"),0,1,'C');

					$this->pdf->SetFont('angsana','',11);
					$this->pdf->Cell(287,4.5,iconv("UTF-8","TIS-620","หลักฐานการบันทึกเวลาเรียน"),0,1,'C');
					//จำนวนสัปห์ดา
					$contentHday=6;
					$this->pdf->SetXY(88,15);
					for($i=0; $i<18; $i++){
						$this->pdf->Cell($contentHday,5,iconv("UTF-8","TIS-620","".($i+1)),1,0,'C');
					}
					...
					$contentWck=3;
					$contentHck=256;//22
					$this->pdf->SetXY(88,20);
					for($i=0; $i<36; $i++){
						$this->pdf->Cell($contentWck,$contentHck,iconv("UTF-8","TIS-620",""),1,0,'C');
					}
					$this->pdf->Ln();
					...
					$this->pdf->Cell(20,7,iconv("UTF-8","TIS-620","รหัสสมัคร"),1,0,'C');
